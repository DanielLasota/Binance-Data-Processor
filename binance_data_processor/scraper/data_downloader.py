...
from binance_data_processor.scraper.scraper_config import AMOUNT_OF_FILES_TO_BE_DOWNLOADED_AT_ONCE
...
from binance_data_processor.utils.file_utils import get_prefix_of_blob_csv_filename
...
class DataDownloader:
    ...
    def __init__(self):
        ...
        if storage_connection_parameters.backblaze_access_key_id is not None:
    ...
    def download_asset_parameters_as_data_frame(self, asset_parameters: AssetParameters) -> pd.DataFrame:
        return self._get_rough_dataframe_from_cloud_storage_files_cut_to_specified_date(asset_parameters)
    ...
    def _get_prefix_of_files_that_should_be_downloaded_for_specified_single_date(asset_parameters: AssetParameters) -> list[str]:
        ...
        target_day_date_prefix = get_prefix_of_blob_csv_filename(asset_parameters=asset_parameters)
        day_date_one_after_prefix = get_prefix_of_blob_csv_filename(asset_parameters=asset_parameters_for_the_next_day) + 'T00-0'
    ...
    def _trade_stream_type_download_handler(self, list_of_files_to_be_downloaded: list[str], market: Market) -> pd.DataFrame:
        import pandas as pd
        from alive_progress import alive_bar

        len_of_files_to_be_downloaded = len(list_of_files_to_be_downloaded)

        records = []
        with alive_bar(len_of_files_to_be_downloaded, force_tty=True, spinner='dots_waves', title='') as bar:
            for i in range(0, len_of_files_to_be_downloaded, AMOUNT_OF_FILES_TO_BE_DOWNLOADED_AT_ONCE):
                batch = list_of_files_to_be_downloaded[i:i + AMOUNT_OF_FILES_TO_BE_DOWNLOADED_AT_ONCE]
                result_queue = queue.Queue()
                threads_list = []
